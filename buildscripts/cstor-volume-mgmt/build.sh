...
echo "==> Removing old bin/cstor-volume-mgmt contents..."
rm -rf bin/cstor-volume-mgmt/*
mkdir -p bin/cstor-volume-mgmt/
...
do
    ...
    do
        output_name="bin/cstor-volume-mgmt/"$GOOS"_"$GOARCH"/"$CTLNAME
...
DEV_PLATFORM="./bin/cstor-volume-mgmt/$(go env GOOS)_$(go env GOARCH)"
for F in $(find ${DEV_PLATFORM} -mindepth 1 -maxdepth 1 -type f); do
    cp ${F} bin/cstor-volume-mgmt/
    cp ${F} ${MAIN_GOPATH}/bin/
done

if [[ "x${VOLUME_MGMT_DEV}" == "x" ]]; then
    # Zip and copy to the dist dir
    echo "==> Packaging..."
    for PLATFORM in $(find ./bin/cstor-volume-mgmt -mindepth 1 -maxdepth 1 -type d); do
