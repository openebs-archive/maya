...
echo "==> Removing old bin/sidecar contents..."
rm -rf bin/sidecar/*
mkdir -p bin/sidecar/
...
do
    ...
    do
        output_name="bin/sidecar/"$GOOS"_"$GOARCH"/"$CTLNAME
...
DEV_PLATFORM="./bin/sidecar/$(go env GOOS)_$(go env GOARCH)"
...
if [[ "x${M_AGENT_DEV}" == "x" ]]; then
    ...
    echo "==> Packaging..."
    for PLATFORM in $(find ./bin/sidecar -mindepth 1 -maxdepth 1 -type d); do
