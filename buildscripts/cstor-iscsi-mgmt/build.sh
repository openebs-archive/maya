...
echo "==> Removing old bin/cstor-iscsi-mgmt contents..."
rm -rf bin/cstor-iscsi-mgmt/*
mkdir -p bin/cstor-iscsi-mgmt/

if [ -z "${CTLNAME}" ];
then
    CTLNAME="cstor-iscsi-mgmt"
fi

# If its dev mode, only build for ourself
if [[ "${ISCSI_MGMT_DEV}" ]]; then
    XC_OS=$(go env GOOS)
...
do
    ...
    do
        output_name="bin/cstor-iscsi-mgmt/"$GOOS"_"$GOARCH"/"$CTLNAME
...
DEV_PLATFORM="./bin/cstor-iscsi-mgmt/$(go env GOOS)_$(go env GOARCH)"
for F in $(find ${DEV_PLATFORM} -mindepth 1 -maxdepth 1 -type f); do
    cp ${F} bin/cstor-iscsi-mgmt/
    cp ${F} ${MAIN_GOPATH}/bin/
done

if [[ "x${ISCSI_MGMT_DEV}" == "x" ]]; then
    # Zip and copy to the dist dir
    echo "==> Packaging..."
    for PLATFORM in $(find ./bin/cstor-iscsi-mgmt -mindepth 1 -maxdepth 1 -type d); do
