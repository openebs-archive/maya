...
var (
	cmdName = "cstor-sidecar"
	usage   = fmt.Sprintf("%s", cmdName)
)

// CstorSidecarOptions defines a type for the options of MayaAgent
type CstorSidecarOptions struct {
	KubeConfig string
	Namespace  string
}
...
func NewCstorSidecar() (*cobra.Command, error) {
	// Define the options for MayaAgent
	options := CstorSidecarOptions{}
...
// Run maya-nodebot
func Run(cmd *cobra.Command, options *CstorSidecarOptions) error {
	glog.Infof("Starting CstorSidecar...")
