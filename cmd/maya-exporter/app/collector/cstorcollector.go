...
// writer writes the "IOSTATS volumename\n" command on the socket.
// it returns the error if the connection is closed or not available
// with socket.
...
func split(resp string) map[string]string {
	...
	result = remove(result, "OK IOSTATS")
