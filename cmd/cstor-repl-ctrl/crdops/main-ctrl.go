...
func CrdOperations(kuberconfig string) {
	...
	// Instantiate the cstor pool and replica controllers.
	poolcontroller := NewCstorPoolController(kubeClient, crdClient, kubeInformerFactory,
		exampleInformerFactory)

	replicacontroller := NewCstorReplicaController(kubeClient, crdClient, kubeInformerFactory,
		exampleInformerFactory)

	go kubeInformerFactory.Start(stopCh)
	go exampleInformerFactory.Start(stopCh)

	// Blocking call for checking status of zrepl in cstor-main container.
	CheckForZrepl()

	// Waitgroup for starting pool and replica controller goroutines.
	...
	go func() {
		if err = replicacontroller.Run(2, stopCh); err != nil {
			glog.Fatalf("Error running cstor controller: %s", err.Error())
		}
		wg.Done()
	}()
