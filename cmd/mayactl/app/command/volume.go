...
const (
	...
	cvrStatus         = "openebs.io/cvr-status"
)
	...
	if fetchAccessMode && volume.Spec.CasType == string(JivaStorageEngine) {
		...
		for _, controllerStatus := range controllerStatuses {
			if controllerStatus != statusRunning {
				return volume, fmt.Errorf("Unable to fetch volume details, Volume controller's status is %q", controllerStatus)
			}
		}
		...
		for _, repl := range collection.Data {
			replica[strings.TrimSuffix(strings.TrimPrefix(repl.Address, "tcp://"), v1.ReplicaPort)] = repl.Mode
		}
