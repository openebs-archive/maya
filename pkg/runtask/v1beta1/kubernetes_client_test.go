...
func fakeGetRuntaskOk(k *kubeclient, cs *internalclientset.Clientset, name string) (*apis.RunTask, error) {
	return &apis.RunTask{}, nil
}

func fakeGetRuntaskErr(k *kubeclient, cs *internalclientset.Clientset, name string) (*apis.RunTask, error) {
...
func TestGet(t *testing.T) {
	tests := map[string]struct {
		...
		getConfig    kube.GetConfigFunc
		getClientset getClientsetFunc
		getRuntask   getRuntaskFunc
		isErr        bool
	}{
		"t10": {"mytask", fakeGetConfigOk, fakeGetClientsetOk, fakeGetRuntaskOk, false},
		"t11": {"", fakeGetConfigOk, fakeGetClientsetOk, fakeGetRuntaskOk, true},
		"t12": {"mytask", fakeGetConfigErr, nil, nil, true},
		"t13": {"mytask", fakeGetConfigOk, fakeGetClientsetErr, nil, true},
		"t14": {"mytask", fakeGetConfigOk, fakeGetClientsetOk, fakeGetRuntaskErr, true},
	}
	for name, mock := range tests {
		t.Run(name, func(t *testing.T) {
			k := &kubeclient{
