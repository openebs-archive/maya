...
func fakeGetCatalogOk(k *kubeclient, cs *internalclientset.Clientset, name string) (*apis.Catalog, error) {
	return &apis.Catalog{}, nil
}

func fakeGetCatalogErr(k *kubeclient, cs *internalclientset.Clientset, name string) (*apis.Catalog, error) {
...
func TestGet(t *testing.T) {
	tests := map[string]struct {
		...
		getConfig    kube.GetConfigFunc
		getClientset getClientsetFunc
		getCatalog   getCatalogFunc
		isErr        bool
	}{
		"t10": {"mytask", fakeGetConfigOk, fakeGetClientsetOk, fakeGetCatalogOk, false},
		"t11": {"", fakeGetConfigOk, fakeGetClientsetOk, fakeGetCatalogOk, true},
		"t12": {"mytask", fakeGetConfigErr, nil, nil, true},
		"t13": {"mytask", fakeGetConfigOk, fakeGetClientsetErr, nil, true},
		"t14": {"mytask", fakeGetConfigOk, fakeGetClientsetOk, fakeGetCatalogErr, true},
	}
	for name, mock := range tests {
		t.Run(name, func(t *testing.T) {
			k := &kubeclient{
