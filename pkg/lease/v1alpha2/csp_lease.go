...
func (sl *Lease) Hold() (interface{}, error) {
	...
	if strings.TrimSpace(leaseValue) == "" ||
		isLeaseExpired(leaseValueObj) ||
		!sl.isLeaderALive(leaseValueObj) {
...
func (sl *Lease) patchCspLeaseAnnotation() error {
	...
	cspPatch, err := patch.NewPatchPayload(PatchOperation,
		PatchPath,
		string(newLeaseValue))
	if err != nil {
...
func (sl *Lease) isLeaderALive(leaseValueObj LeaseContract) bool {
	...
	pod, err := sl.Kubeclientset.
		CoreV1().
		Pods(podDetails[0]).
		Get(podDetails[1], meta_v1.GetOptions{})
	if err != nil {
