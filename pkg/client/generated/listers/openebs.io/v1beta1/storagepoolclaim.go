...
type StoragePoolClaimLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.StoragePoolClaim, err error)
	// Get retrieves the StoragePoolClaim from the index for a given name.
	Get(name string) (*v1beta1.StoragePoolClaim, error)
...
// List lists all StoragePoolClaims in the indexer.
func (s *storagePoolClaimLister) List(selector labels.Selector) (ret []*v1beta1.StoragePoolClaim, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.StoragePoolClaim))
	})
	return ret, err
}

// Get retrieves the StoragePoolClaim from the index for a given name.
func (s *storagePoolClaimLister) Get(name string) (*v1beta1.StoragePoolClaim, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("storagepoolclaim"), name)
	}
	return obj.(*v1beta1.StoragePoolClaim), nil
}
