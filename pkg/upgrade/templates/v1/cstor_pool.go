...
var (
    ...
    "metadata": {
        ...
        }{{if compareVersions .CurrentVersion "1.9.0"}},
        "finalizers": [
            "openebs.io/pool-protection"
        ]{{end}}
    },{{if compareVersions .CurrentVersion "1.8.0"}}
    "spec": {
        "poolSpec": {
            "roThresholdLimit": 85
        }
        ...
        "containers": [
          {
            ...
            "livenessProbe": {
              ...
              "successThreshold": 1,
              "timeoutSeconds": 150
            }{{if compareVersions .CurrentVersion "1.7.0"}},
            "volumeMounts": [
              {
                "name": "storagepath",
          ...
          },
          {
            ...
            "image": "{{.PoolMgmtImage}}:{{.ImageTag}}"{{if compareVersions .CurrentVersion "1.7.0"}},
            "volumeMounts": [
              {
                "name": "storagepath",
                "mountPath": "/var/openebs/cstor-pool"
          ...
          {
            "name": "maya-exporter",
            "image": "{{.MExporterImage}}:{{.ImageTag}}"{{if compareVersions .CurrentVersion "1.7.0"}},
            "volumeMounts": [
              {
                "name": "storagepath",
                "mountPath": "/var/openebs/cstor-pool"
