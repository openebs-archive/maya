...
func (v *Operation) getCloneLabels() (map[string]interface{}, error) {
	...
	if v.volume.CloneSpec.IsClone {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "failed to retrieve source volume for clone: %s", v.volume)
		}
...
func (v *Operation) Delete() (*v1alpha1.CASVolume, error) {
	...
	if len(castName) == 0 {
		return nil, errors.Errorf("failed to delete volume: missing cas template: %s", v.volume)
	}
...
func NewListOperation(volumes *v1alpha1.CASVolumeList) (*ListOperation, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "failed to instantiate volume list operation")
	}
...
func (v *ListOperation) List() (*v1alpha1.CASVolumeList, error) {
	...
	for _, castName := range strings.Split(castNames, ",") {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "failed to list volumes: cas template '%s'", castName)
		}
