...
// getClientset returns a new instance of runtask based clientset
//
// NOTE:
...
// getOpenebsClusterFunc abstracts fetching an OpenebsCluster
// instance
//
// NOTE:
//  function type makes it simple to mock
type getOpenebsClusterFunc func(k *kubeclient, cs *internalclientset.Clientset, name string) (*apis.OpenebsCluster, error)

// getOpenebsCluster returns an instance of OpenebsCluster
// corresponding to the provided name
//
// NOTE:
//  This is an implementation of getOpenebsClusterFunc
func getOpenebsCluster(k *kubeclient, cs *internalclientset.Clientset, name string) (*apis.OpenebsCluster, error) {
	if k == nil {
	...
	return cs.OpenebsV1alpha1().OpenebsClusters(k.namespace).Get(name, v1.GetOptions{})
}

// kubeclient enables kubernetes API operations on runtask instance
type kubeclient struct {
	*kube.Client                                   // embeds kubernetes client related functions
	getConfig         kube.GetConfigFunc           // handle to get kubernetes config
	getClientset      getClientsetFunc             // handle to get clientset to invoke API calls against OpenebsCluster
	getOpenebsCluster getOpenebsClusterFunc        // handle to get OpenebsCluster instance
...
func withDefaults(k *kubeclient) {
	...
	if k.getOpenebsCluster == nil {
		k.getOpenebsCluster = getOpenebsCluster
	}
}
...
func (k *kubeclient) getInternalClientset() (*internalclientset.Clientset, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to get openebscluster clientset")
	}
...
// Get returns a OpenebsCluster instance from kubernetes cluster
func (k *kubeclient) Get(name string) (*apis.OpenebsCluster, error) {
	if strings.TrimSpace(name) == "" {
		return nil, errors.New("failed to get openebscluster: missing openebscluster name")
	}
	cs, err := k.getCachedOrNewInternalClientset()
	if err != nil {
		return nil, err
	}
	return k.getOpenebsCluster(k, cs, name)
}
