...
// kubeclient enables kubernetes API operations on openebs
// cluster instance
...
// KubeClient returns a new instance of kubeclient meant
// for openebs cluster operations
...
// implements openebs cluster Interface
...
// Get returns an openebs cluster instance from kubernetes
// cluster
func (k *kubeclient) Get(name string, opts ...provider.GetOptionFn) (*apis.OpenebsCluster, error) {
	if strings.TrimSpace(name) == "" {
		return nil, errors.New("failed to get openebs cluster instance: missing name")
	}
	...
	var oc apis.OpenebsCluster
	key := client.ObjectKey{Namespace: getopt.Namespace, Name: name}
	err = k.GetFn(cli, context.TODO(), key, &oc)
	if err != nil {
		return nil, err
	}
	return &oc, nil
