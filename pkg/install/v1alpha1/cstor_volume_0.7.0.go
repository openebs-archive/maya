...
spec:
  ...
  post: |
    ...
    {{- jsonpath .JsonResult "{.items[*].status.phase}" | trim | saveAs "readlistrep.cvrstatus" .TaskResult | noop -}}
  ...
  task: |
    ...
    metadata:
      ...
      annotations:
        ...
        openebs.io/cvr-status: {{ .TaskResult.readlistrep.cvrstatus | default "" | splitList " " | join "," }}
    spec:
      capacity: {{ $capacity }}
      iqn: iqn.2016-09.com.openebs.cstor:{{ .Volume.owner }}
