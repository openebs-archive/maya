...
var _ = Describe("Integration Test", func() {
	...
	// Test Case #3 (sparse-mirrored-auto-spc). Type : Positive
	When("We apply sparse-mirrored-auto spc yaml with maxPool count equal to 1 on a k8s cluster having at least 1 capable node", func() {
		It("pool resources count should be 1 with no error and online status", func() {
			// TODO: Create a generic util function in utils.go to convert yaml into go object.
			// ToDo: More POC regarding this util converter function.
			// Functions generic to both cstor-pool and cstor-vol should go inside common directory
			...
			Expect(cspCount).To(Equal(1))
			// We expect 1 storagePool objects.
			var spCount int
			maxRetry = 10
			for i := 0; i < maxRetry; i++ {
				spCount, err = getStoragePoolCount(spcGot.Name, citfInstance)
			...
			Expect(spCount).To(Equal(1))

			// We expect 'online' status on one cstorPool object(i.e. 1 online counts)
			var onlineCspCount int
			maxRetry = 10
			for i := 0; i < maxRetry; i++ {
				onlineCspCount, err = getCstorPoolStatus(spcGot.Name, citfInstance)
			...
			Expect(onlineCspCount).To(Equal(1))
